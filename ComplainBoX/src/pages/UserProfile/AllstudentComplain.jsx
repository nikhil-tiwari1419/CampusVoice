...
import { getComplain, deleteComplain } from "../../api/user";
import ComplaintVoteModal from "../../components/ComplainVotingpage"
...
export default function AllComplaints() {
  ...
  async function handleDelete(complainId) {
    ...
    try {
      ...
      alert(error.response?.data?.message || "Unable to delete this complaint.");
    }
  }

  // called by the vote modal after a successful vote — keeps the list in sync
  function handleVoteUpdate(complaintId, { voted, voteCount }) {
    setComplaints((prev) =>
      prev.map((c) =>
        c.id === complaintId ? { ...c, hasVoted: voted, voteCount } : c
      )
    );
    setSelectedComplaint((prev) =>
      prev && prev.id === complaintId ? { ...prev, hasVoted: voted, voteCount } : prev
    );
  }
            ...
            {filteredComplaints.map((c) => {
              ...
              return (
                <div
                  ...
                  className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-lg shadow-slate-200/40 backdrop-blur-xl hover:border-slate-300 hover:shadow-xl transition-all space-y-3 cursor-pointer"
                >
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold border ${statusStyles[c.status] || statusStyles.Pending
                          }`}
                      >
                        <StatusIcon className="w-3.5 h-3.5" />
                        {c.status}
                      ...
                      <button
                        className={`inline-flex items-center py-3 px-3 cursor-pointer rounded-lg text-xs font-semibold border ${statusStyles.Delete}`}
                        onClick={(e) => {
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <span className="inline-flex items-center gap-1.5 text-xs text-slate-500">
                      <ThumbsUp className={`w-3.5 h-3.5 ${c.hasVoted ? "text-teal-600 fill-teal-600" : ""}`} />
                      {c.voteCount} vote{c.voteCount !== 1 ? "s" : ""}
                    </span>
                    <span className="text-[11px] text-slate-400">Click to view & vote</span>
                  </div>
