  ...
  return (
    <div className="relative min-h-screen w-full bg-white text-slate-900 font-sans overflow-hidden select-none py-12 px-4 sm:px-6">
      ...
      <div className="relative max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-2xl shadow-slate-200/40 backdrop-blur-xl">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-200">
            <div className="flex items-center gap-4">
              <div className="h-16 w-16 shrink-0 rounded-2xl bg-gradient-to-tr from-teal-500/20 to-teal-400/10 border border-teal-500/30 text-teal-600 flex items-center justify-center text-2xl font-bold uppercase shadow-inner">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isProfileComplete ? (
                    <span className="inline-flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 px-2.5 py-0.5 rounded-full text-[11px] font-semibold">
                      <CheckCircle2 className="w-3 h-3" />
                      Verified & Locked
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 bg-amber-500/10 border border-amber-500/20 text-amber-600 px-2.5 py-0.5 rounded-full text-[11px] font-semibold">
                      <Clock className="w-3 h-3" />
                      Setup Pending
                    </span>
                  )}
                </div>
                <p className="text-xs text-slate-600 mt-0.5">{formData.email}</p>
              </div>
            </div>

            {isProfileComplete ? (
              <div className="inline-flex items-center gap-2 bg-slate-50 text-slate-700 px-3.5 py-2 rounded-xl text-xs font-semibold border border-slate-200 shadow-inner">
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 hover:border-slate-300 focus:border-teal-400/80 focus:ring-2 focus:ring-teal-400/20 rounded-xl py-2.5 px-3 text-sm text-slate-900 outline-none transition-all disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
                >
                  {programOptions.map((prog) => (
                    <option key={prog.name} value={prog.name} className="bg-white text-slate-900">
            ...
            <div>
              ...
              {formData.program === "BCA" ? (
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 text-sm text-slate-500 outline-none opacity-60 cursor-not-allowed"
                />
              ) : (
                <select
                  name="branch"
            ...
            <div className="sm:col-span-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange}
                  disabled={isProfileComplete || saving}
                  className="w-full bg-slate-50 border border-slate-200 hover:border-slate-300 focus:border-teal-400/80 focus:ring-2 focus:ring-teal-400/20 rounded-xl py-2.5 pl-12 pr-3 text-sm font-mono text-slate-900 placeholder-slate-400 outline-none transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                />
              </div>
          ...
          {!isProfileComplete ? (
            ...
            <div className="mt-6 pt-4 border-t border-slate-200 flex items-center gap-2.5 text-xs text-slate-600">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-7 shadow-xl shadow-slate-200/40 backdrop-blur-xl">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link
              to="/complaints"
              className="text-xs font-medium text-teal-600 hover:text-teal-500 flex items-center gap-1 transition-colors"
            >
          ...
          {loadingComplaints ? (
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-8 text-center">
              <CheckCircle2 className="w-8 h-8 text-teal-600/50 mx-auto mb-2" />
              <p className="text-sm font-medium text-slate-700">No complaints registered yet</p>
            ...
            <div className="space-y-3">
              {recentComplaints.map((item, index) => {
                ...
                return (
                  <div
                    key={item._id || index}
                    className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 hover:border-slate-300 transition-all"
                  >
                    ...
                    <Link
                      to="/complaints"
                      className="text-xs text-slate-500 hover:text-teal-600 flex items-center gap-1 transition-colors shrink-0"
                    >
                      <span>Track</span>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-lg bg-white border border-amber-500/40 rounded-2xl p-6 sm:p-7 shadow-2xl shadow-slate-200/50 text-slate-900 space-y-5">
            {/* Top Warning Badge & Title */}
            <div className="flex items-start gap-3.5">
              <div className="w-11 h-11 rounded-xl bg-amber-500/20 border border-amber-500/30 text-amber-600 flex items-center justify-center shrink-0">
            ...
            {/* Warning Notice Box */}
            <div className="bg-amber-500/10 border border-amber-500/25 rounded-xl p-3.5 text-xs text-amber-900/90 leading-relaxed">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2.5 text-xs">
              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider pb-1 border-b border-slate-200">
                Summary of Credentials to Lock
              </p>
              <div className="flex justify-between py-1 border-b border-slate-200">
                <span className="text-slate-600">Degree Program:</span>
                <span className="font-semibold text-teal-600">{formData.program}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-200">
                <span className="text-slate-600">Branch / Specialization:</span>
              ...
              <div className="flex justify-between py-1 border-b border-slate-200">
                ...
                <span className="font-semibold text-slate-800">Semester {formData.sem}</span>
              </div>
              <div className="flex justify-between py-1">
                <span className="text-slate-600">Mobile Number:</span>
                <span className="font-semibold font-mono text-slate-800">+91 {formData.phone}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-2.5 pt-2">
              <button
                ...
                disabled={saving}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-xs font-semibold border border-slate-300 transition-all cursor-pointer disabled:opacity-50"
              >
